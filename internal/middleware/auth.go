...
func AdminOnly() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if !ok || userRole != "admin" {
			...
			logger.LogSecurityEvent("unauthorized_admin_access", getUserID(userID), c.ClientIP(), map[string]interface{}{
				"role": userRole,
				"path": c.Request.URL.Path,
			})
...
func RequirePermission(permission string) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if !auth.HasPermission(userRole, permission) {
			userID, _ := c.Get("user_id")
			logger.LogSecurityEvent("insufficient_permissions", getUserID(userID), c.ClientIP(), map[string]interface{}{
				"role":                userRole,
...
// RequireRole middleware requires specific role or above
func RequireRole(requiredRole string) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if !auth.HasRoleOrAbove(userRole, requiredRole) {
			userID, _ := c.Get("user_id")
			logger.LogSecurityEvent("insufficient_role", getUserID(userID), c.ClientIP(), map[string]interface{}{
				"role":          userRole,
			...
			c.JSON(http.StatusForbidden, gin.H{
				...
				"error": map[string]string{
					...
					"message": "This endpoint requires higher role privileges",
				},
			})
			c.Abort()
			return
